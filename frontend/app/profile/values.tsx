...
export default function Values() {
  ...
  const saveAudioFile = async (uri: string | null) => {
    ...
    const fileName = `values_audio_${Date.now()}.m4a`;
  ...
  const saveTranscription = async (text: string) => {
    ...
    const fileName = `values_transcription_${Date.now()}.txt`;
  ...
  const saveCSVFromTranscription = async (text: string) => {
    ...
    const fileName = `values_words_${Date.now()}.csv`;
  ...
  return (
    <View style={styles.container}>
      <View style={styles.card}>
        <Text style={styles.title}>Tell us about your values</Text>
